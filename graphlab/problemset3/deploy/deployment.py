...
def run_benchmark(n, i_type, bench_args): 
    ...
    try:
        ...
        # Copy code        
        print 'Updating app'
        run_cmd(master, 'mkdir graphlab/apps/550')
        put_file(master, '../code/CMakeLists.txt', 'graphlab/apps/550/CMakeLists.txt')
        put_file(master, '../code/main.cpp', 'graphlab/apps/550/main.cpp')

        # config
        print "Configuring"
        run_cmd(master, 'cd graphlab;./configure;cd ..')
        run_cmd(master, 'sudo chmod 777 /home/ubuntu/graphlab/deps/hadoop/src/hadoop/src/c++/libhdfs/configure')
